...
class SignalProcessor:
    ...
    def fitDampedSineWave(self, amplitudeParam = 1.0, frequencyParam = 10.0, phaseParam = 0.0, decayRateParam = 0.1):
        """
        Fit a damped sine wave to the filtered signal using nonlinear least squares.
        :param amplitudeParam: Initial guess for the amplitude of the sine wave.
        :param frequencyParam: Initial guess for the frequency of the sine wave.
        :param phaseParam: Initial guess for the phase shift of the sine wave.
        :param decayRateParam: Initial guess for the decay rate of the sine wave.
        """
        ...
        initialParams = np.array([amplitudeParam, frequencyParam, phaseParam, decayRateParam])

        bounds = ([0, 0, -np.pi, 0], [np.inf, np.inf, np.pi, np.inf])       # apply bounds based on physical or theoretical nature

        try:
            result = optimize.least_squares(residualsDamped, initialParams, bounds=bounds, args=(self.timeVector, self.filteredSignal))
